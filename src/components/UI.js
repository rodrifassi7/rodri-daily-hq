...
export const Card = ({ children, className, style, onClick }) => (_jsx("div", { onClick: onClick, style: style, className: cn("bg-card border border-border rounded-2xl p-4 mb-4", onClick && "active:scale-95 transition-transform", className), children: children }));
...
export const Badge = ({ children, color = 'bg-primary/20 text-primary', className, onClick }) => (_jsx("span", { onClick: onClick, className: cn("px-2 py-0.5 rounded-full text-[10px] font-bold uppercase tracking-wider", color, onClick && "cursor-pointer active:scale-95 transition-all", className), children: children }));
